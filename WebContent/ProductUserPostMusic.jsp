...
<div>
        ...
        <hr size="" class="netscape4" />
       	...
       	<%
               	if(!authorizationPageBeanId.getStrMessage().equals("")){
               	%>
       	<SCRIPT LANGUAGE="javascript">
         alert('<%= authorizationPageBeanId.getStrMessage() %>');
		</SCRIPT>
      	<%
...
<table class="columns">
    ...
    <tbody>
		...
		<br/>
        <form method="post" name="postsoftform"  ACTION="ProductUserPostMusic.jsp" onSubmit="return  IsFormOk()"  >
		<br/>
		<br/>
		<div class="box">
		  <div class="body">
		    ...
		    <DIV style="background-image:url('images/f.jpg');height:20px; TEXT-ALIGN: left"  >
				<font color='white' size='2' > &nbsp;&raquo; <%=authorizationPageBeanId.getLocalization(application).getString("music_form_describe_extend_field_for_music_info")%> </font>
                     ...
                     <TR><TD><%=authorizationPageBeanId.getLocalization(application).getString("upload_audio_clip")%>:* </TD> <TD><input  name="bigimagename" disabled="disabled" size="20" value="<%= publisherBeanId.getBigimgname() %>" ><input type="button" name="newbig_image" value="<%= authorizationPageBeanId.getLocalization(application).getString("upload_audio_clip") %>"  onclick="dwindow('NewAudioClip.jsp'); return false;" ><input type="button" name="selectbig_image" value="<%= authorizationPageBeanId.getLocalization(application).getString("select_uploaded_music") %>" image" onclick="dwindow('SelectBigImage.jsp'); return false;" ><input type="hidden"  name="bigimage_id" size="20" value="<%= publisherBeanId.getBigimage_id() %>" ></TD></TR>
                     <TR><TD><%=authorizationPageBeanId.getLocalization(application).getString("full_information")%>:* </TD> <TD> <textarea name="fulldescription" rows="10" cols="70"  ><%=publisherBeanId.getProduct_fulldescription()%></textarea></TD></TR>
                      <TR><TD><%=authorizationPageBeanId.getLocalization(application).getString("attache_atbom")%>:* </TD> <TD><input  name="filename" disabled="disabled"  size="20" value="<%= publisherBeanId.getFilename() %>" ><input type="button" name="newfile" value="<%= authorizationPageBeanId.getLocalization(application).getString("new_file") %>" onclick="dwindow('NewFileMusicArchive.jsp')" ><input type="button" name="selectfile" value="<%= authorizationPageBeanId.getLocalization(application).getString("select_file") %>" onclick="dwindow('SelectFile.jsp')" ><input type="hidden"  name="file_id" size="20" value="<%= publisherBeanId.getFile_id() %>" ></TD></TR>
