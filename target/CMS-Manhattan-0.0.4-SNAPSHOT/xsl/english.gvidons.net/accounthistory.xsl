...
...
	    ...
	    <div class="description"><img src="xsl/english.gvidons.net/img/basket1.gif"/>Searching order by account number <FORM name="order" action="Order.jsp" ><INPUT SIZE="0"  AUTOCOMPLETE="off" TYPE="TEXT" NAME="account_history_id"></INPUT><INPUT TYPE="submit" name="submit" value="Search"></INPUT></FORM></div>
		  ...
		  <div class="body">
			...
			<table >
			    <tbody>
                       ...
                       <TR><TD width="50" >Invoice</TD><TD width="30" align="right"  >Amount</TD><TD width="50"></TD><TD width="200" >Date</TD><TD width="250" >Payment status </TD><TD></TD></TR>
			...
			<xsl:for-each select="document/list/payment">
				...
				<xsl:if test="complete = 't'"> <STRONG><FONT color="#000099">Выполненная</FONT></STRONG></xsl:if><xsl:if test="complete = 'f'"> <STRONG><FONT color="#000099">Ожидаем выполнения</FONT></STRONG></xsl:if>
                                ...
                                <TD align="right"  ><FORM name="order" action="AccountHistoryDetal.jsp" ><INPUT SIZE="0"  AUTOCOMPLETE="off" TYPE="HIDDEN" NAME="amount_id"><xsl:attribute name="value"><xsl:value-of select="amount_id"/></xsl:attribute></INPUT><INPUT TYPE="submit" name="submit" value="Detail"></INPUT></FORM></TD>
                                </TR>
			</xsl:for-each>
