...
public class OperationAmountBean implements java.io.Serializable  {
	...
	public float getConvCurrency(String strCurrency_IDFrom,
			String strCurrency_IDTo) {
		float floRate = 0;
		String strRezult = "";
		...
		// " + intUserID ;
		String query = "SELECT " + strCurrency_IDTo
				+ " FROM currency_converter WHERE currency_id = "
				+ strCurrency_IDFrom;
	...
	public String setBuy(String soft_id, int intUserID, String order_id) {
		...
		//String query = "SELECT NEXT VALUE FOR account_hist_id_seq  AS ID  FROM ONE_SEQUENCES";
		...
		try 
		{
			...
			query = "SELECT  account.amount, account.currency_id , currency.currency_lable, currency.currency_desc FROM account LEFT OUTER JOIN currency ON account.currency_id = currency.currency_id WHERE account.user_id = "	+ intUserID;
		...
		// \"soft\".\"soft_id\" = " + soft_id ;
			query = "SELECT  soft.name, soft.description , soft.version, soft.cost , soft.currency, soft.serial_nubmer , catalog.lable , catalog.tax  FROM soft LEFT  JOIN catalog  ON soft.catalog_id = catalog.catalog_id WHERE soft.soft_id = "	+ soft_id;
			Adp.executeQuery(query);
		...
		//floRezult_amount = Float.parseFloat(old_amount)	- ( Float.parseFloat(strCostProduct) * floRate);
		...
		query = "insert into account_hist " + "(" + " id  , " + " user_id , "
				+ " order_id , " + " add_amount , " + " old_amount  , "
				+ " date_input , " + " complete   , " + " decsription  , "
				+ " currency_id_add  , " + " currency_id_old  , "
				+ " currency_id_total  , " + " active  , " + " rezult_cd , "
				+ " date_end , " + "account_hist.sysdate , " + " total_amount  , "
				+ " tax  , " + " withtax_total_amount  " + ")" + " VALUES "
				+ "( " + strID + ", " + intUserID + ", " + order_id + ", "
				+ strCostProduct + ", " + old_amount + ", " + "now()" + ", "
				+ "true" + ", '" + "Bought product " + strProductName + ", "
				+ strDescriptionProduct + ", version " + strVersionProduct
				+ "', " + strCurrency_IDProduct + ", " + strCurrency_IDAccount
				+ ", " + strCurrency_IDAccount + ", " + "false  , '" + "bought"
				+ "', " + "now()" + ", " + "now()" + ", " + floRezult_amount
				+ ", " + strTaxProduct + ", " + floWithtax_rezult_amount + " "
				+ ")";
	...
	// int intUserID ) {
	public String addMoneyStart(String strDescriptionProduct,
			double amountDebit, String strCurrency_IDAmountDebit,
		...
		try {
			...
			query = "insert into account_hist " + "(" + " id  , "
					...
					+ " old_amount  , " + " date_input , " + " complete   , "
					+ " decsription  , " + " currency_id_add  , "
					+ " currency_id_old  , " + " currency_id_total  , "
					+ " active  , " + " account_hist.sysdate , " + " total_amount , "
					+ " tax  , " + " withtax_total_amount ," + " user_ip ,"
					+ " user_header ," + " rate " + ")" + " VALUES " + "( "
					+ strID + ", " + intUserID + ", " + order_id + ", "
					+ amountDebit + ", " + old_amount + ", " + "now()"
					+ ", " + "false" + ", '" + strDescriptionProduct + "', "
					+ strCurrency_IDAmountDebit + ", " + strCurrency_IDAccount
					+ ", " + strCurrency_IDAccount + ", " + "true  , "
					+ "now()" + ", " + rezult_amount + ", " + taxProduct
					+ ", " + withtax_rezult_amount + ", '" + user_ip
					+ "', '" + user_header + "', " + 0  + " " + ")";
		...
		/*
		 * query = "UPDATE account SET complete = false , amount = " +
		 * floRezult_amount + " WHERE account.user_id = " + intUserID ;
		 * 
		 * try{ Adp.executeUpdate(query); } catch (SQLException ex) {
		 * System.err.println(query); System.err.println(ex);
		 * System.err.println("" + this.getClass()); System.err.println("Method " +
		 * "addMoneyStart()"); Adp.rollback(); Adp.close(); return null ; }
		 */
	 ...
	 * QueryManager Adp = new QueryManager("jdbc:postgresql://192.168.0.1:5432/mobilsoft") ;
	 ...
	 * strAccount_Hist_ID ; try { Adp.executeUpdate(query); } catch
	 * (SQLException ex) { System.err.println(query); System.err.println(ex);
	 * System.err.println("" + this.getClass()); System.err.println("Method " +
