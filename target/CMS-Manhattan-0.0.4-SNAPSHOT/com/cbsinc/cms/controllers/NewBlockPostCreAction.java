			 ...
			 if( SoftPostBeanId.getAction().compareTo("save") == 0 )
			 {
				   ...
				   if(SoftPostBeanId.getSoft_id().compareTo("-1")==0)
				   {
					   ...
					   // response.sendRedirect("Productlist.jsp?offset=" + 0 + "&catalog_id=" + AuthorizationPageBeanId.getCatalog_id()  );
					    response.sendRedirect("Productlist.jsp?offset=" + 0 + "&catalog_id=" + AuthorizationPageBeanId.getCatalogParent_id());
				   ...
				   else 
				   {
					   ...
					   response.sendRedirect("Productlist.jsp?offset=" + AuthorizationPageBeanId.getOffsetLastPage()+ "&catalog_id=" + AuthorizationPageBeanId.getCatalogParent_id());
				   }
			 }
...
//				 end novigator action ---
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response, ServletContext servletContext) throws Exception 
	{
		...
		SoftPostBeanId.setNameOfPage("ProductPostCre.jsp");
	...
	public void action(HttpServletRequest request, HttpServletResponse response, ServletContext servletContext) throws Exception 
	{
		...
		//|| catalog_editBean == null || catalog_addBean == null 
...
//		 Start Novigator ---
		...
		else catalog_listBean.setOffset(0);
...
//		else SoftPostBeanId.setSave("false"); 
		...
		if ( softname2 != null ) {  SoftPostBeanId.setStrSoftName2( softname2 ); }

		String   jsp_url  = request.getParameter("jsp_url");
		if ( jsp_url != null ) {  SoftPostBeanId.setJsp_url( jsp_url ); }
		...
		if ( file_id != null ) {  SoftPostBeanId.setFile_id( file_id ); }
