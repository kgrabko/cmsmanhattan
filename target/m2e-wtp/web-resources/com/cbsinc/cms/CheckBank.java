...
public class CheckBank implements java.io.Serializable  {
	...
	public String CheckOrder(OrderBank objOrderBank) {
		...
		try {
			...
			InputStreamReader inputstreamreader = new InputStreamReader(url1
					.openStream());
			...
			BufferedReader bufferedreader = new BufferedReader(
					inputstreamreader);
			bufferedreader = postData(
					"http://secure.assist.ru/results/results_long.cfm", request);
			String nextLine = "";
	...
	synchronized public void parserRequest(String i_strNumerOrder,
			String i_strRezult, String i_strDecsription) {
		System.out.println("Order: " + i_strNumerOrder + " Rezalt: "
				+ i_strRezult + " " + i_strDecsription);
		QueryManager Adp = new QueryManager();
	...
	public String[] tokenize(String s, String d) {
		...
		for (StringTokenizer stringtokenizer = new StringTokenizer(s, d); stringtokenizer.hasMoreTokens(); vector.addElement(stringtokenizer.nextToken()))	;
	...
	void end_addmoney(QueryManager Adp, String i_strAccountHistory_id,
		...
		query = "UPDATE account SET amount = " + total_amount + " , curr = "
				+ rate + " , date_input = '" + date_input
