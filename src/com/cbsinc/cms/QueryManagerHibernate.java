...
import com.cbsinc.dao.utils.HibernateUtil;
	...
	public class QueryManagerHibernate implements java.io.Serializable , IQueryManager {
		...
		final public void executeQuery(String query) throws SQLException {
			...
			try 
			{
				...
				Object	obj = it.next();
				if(obj instanceof Object[] )  row = (Object[])obj;
				else
				{
					row = new Object[1] ;
					row[0] = obj ;
	...
	final public int executeInsertWithArgs(String query  , Object[] args ) throws SQLException 
		{
			...
			try 
			{
				...
				{
					if( args[i] instanceof String ) upadteQuery.setString(i + 1, (String)args[i]) ;
					else if( args[i] instanceof Integer ) upadteQuery.setInteger(i +1, (Integer)args[i]) ;
					else if( args[i] instanceof Long ) upadteQuery.setLong(i + 1, (Long)args[i]) ;
					else if( args[i] instanceof Float ) upadteQuery.setFloat(i +1, (Float)args[i]) ;
					else if( args[i] instanceof Double ) upadteQuery.setDouble(i + 1, (Double)args[i]) ;
					else if( args[i] instanceof Byte ) upadteQuery.setByte(i + 1, (Byte)args[i]) ;
					else if( args[i] instanceof Time ) upadteQuery.setTime(i + 1, (Time)args[i]) ;
					else if( args[i] instanceof java.sql.Date ) upadteQuery.setDate(i + 1 , (java.sql.Date)args[i]) ;
					else if( args[i] instanceof Object ) upadteQuery.setSerializable(i + 1, (Serializable)args[i]) ;
				}
		...
		final public int executeInsertWithArgs(String query  , Map args ) throws SQLException 
		{
			...
			try 
			{
				...
				for(int i = 0 ; i < fields.length ; i++ )
				{
					 ...
					 if( obj instanceof String ) upadteQuery.setString(i + 1, (String)obj) ;
						else if( obj instanceof Integer ) upadteQuery.setInteger(i +1, (Integer)obj) ;
						else if( obj instanceof Long ) upadteQuery.setLong(i + 1, (Long)obj) ;
						else if( obj instanceof Float ) upadteQuery.setFloat(i +1, (Float)obj) ;
						else if( obj instanceof Double ) upadteQuery.setDouble(i + 1, (Double)obj) ;
						else if( obj instanceof Byte ) upadteQuery.setByte(i + 1, (Byte)obj) ;
						else if( obj instanceof Time ) upadteQuery.setTime(i + 1, (Time)obj) ;
						else if( obj instanceof java.sql.Date ) upadteQuery.setDate(i + 1 , (java.sql.Date)obj) ;
						//else if( obj instanceof java.util.Date ) prepa_stat.setDate(i + 1 , new java.sql.Date(((java.util.Date)obj)).getTime()) ;
						else if( obj instanceof java.util.Date ) upadteQuery.setDate(i + 1 , new java.sql.Date(((java.util.Date)obj).getTime())) ;
						else if( obj instanceof Object ) upadteQuery.setSerializable(i + 1, (Serializable)obj) ;
						else if( obj == null ) upadteQuery.setSerializable(i + 1, (Serializable)obj) ;
				}
...
//		}
		...
		final public void executeQueryWithArgs(String query  , Object[] args ) throws SQLException 
		{
			...
			try 
			{
				...
				for(int i = 0 ; i < args.length ; i++ )
				{
					if( args[i] instanceof String ) hquery.setString(i + 1, (String)args[i]) ;
					else if( args[i] instanceof Integer ) hquery.setInteger(i +1, (Integer)args[i]) ;
					else if( args[i] instanceof Long ) hquery.setLong(i + 1, (Long)args[i]) ;
					else if( args[i] instanceof Float ) hquery.setFloat(i +1, (Float)args[i]) ;
					else if( args[i] instanceof Double ) hquery.setDouble(i + 1, (Double)args[i]) ;
					else if( args[i] instanceof Byte ) hquery.setByte(i + 1, (Byte)args[i]) ;
					else if( args[i] instanceof Time ) hquery.setTime(i + 1, (Time)args[i]) ;
					else if( args[i] instanceof java.sql.Date ) hquery.setDate(i + 1 , (java.sql.Date)args[i]) ;
					else if( args[i] instanceof Object ) hquery.setSerializable(i + 1, (Serializable)args[i]) ;
				}
		...
		final public void rollback() {
			...
			try {
				if (session == null) throw new SQLException("rollback , Session == null, query=" + Qtable);
				if (!session.getTransaction().isActive()) throw new SQLException("rollback, Hebernate Transaction must be used, query=" + Qtable);
		...
		final public void commit() {
			try 
			{
				if (session == null) throw new SQLException("commit, Connention == null, query=" + Qtable);
				if (!session.getTransaction().isActive()) throw new SQLException("commit, Hebernate Transaction must be used, query=" + Qtable);
		...
		final public void BeginTransaction() {
			...
			try {
				if (session == null) throw new SQLException("BeginTransaction , Session == null, query=" + Qtable);
