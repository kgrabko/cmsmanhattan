...
public class Creteria_listBean implements java.io.Serializable  { 
	...
	public void initPage(String addstring)
	{
		...
		List list = null ;
		String query = "";
		query = "select "  +  table_name +  "_id , name , label  FROM "  +  table_name +  " WHERE active = true " + addstring + "  and ( link_id = 0 or link_id = " + link_id + " ) limit 10 offset " + offset;
		try 
		{
			list = Adp.executeQueryList(query);
			listDataGet.addList(list);
			
			if(listDataGet.getRowCount() > 0) title = listDataGet.getValueAt(0, 2);
		} 
	...
	public String getTable_old( String addstring ) 
	{
		...
		QueryManager Adp = new QueryManager();
		String query = "";
		query = "select "  +  table_name +  "_id , name , label  FROM "  +  table_name +  " WHERE active = ? " + addstring + "  and ( link_id = 0 or link_id = ? ) " ;
		try 
		{
		...
		if (Adp.rows().size() > 0){ title = (String) Adp.getValueAt(0, 2); }
		...
		table.append("<TR>" + "<TD></TD>" + "<TD></TD>" + "<TD><a href=\""	+ listdown + "\">назад 10</a>  </TD>" + "</TR>\n");
		table.append("</tbody>\n");
		table.append("</TABLE>\n");
		} 
		catch (SQLException ex) 
	...
	{
		if(catalog_id.startsWith("-") || catalog_id.equals("0") ) return ;
		QueryManager Adp = new QueryManager();
		String query = "";
