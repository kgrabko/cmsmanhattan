...
public class CurrencyHash implements java.io.Serializable  {
	...
	static public CurrencyHash getInstance()
	{
		 synchronized (CurrencyHash.class) {
			 if( currencyHash == null ) currencyHash = new CurrencyHash()  ;
		}
	...
	private void init()
	{
		...
		try 
		{
			...
			for(int i =0 ; i < Adp.rows().size() ; i++ )
			{
				...
				hashTable.put(currency_id, new Currency(currency_id,currency_lable,currency_desc,rate,currency_cd,cursdate ) );
			}
		} 
		catch (Exception ex) 
		{
	...
	public  Currency getCurrency(String currency_id) { 
		...
		if(curr == null) 
		{
			...
			log.error("ERROR: mobilesoft.CurrencyHash.getCurrency(String currency_id) and currency_id = " + currency_id);
		}
		return curr;
	}
	...
	private  String getCurrency_id(String currency_code) 
	{
		...
		{
			if(((Currency)currency).getCode().compareTo(currency_code) == 0) return ((Currency)currency).getCode() ;
		}
	...
	{
		if(hashTable.get(currency_id) == null) return "" ;
